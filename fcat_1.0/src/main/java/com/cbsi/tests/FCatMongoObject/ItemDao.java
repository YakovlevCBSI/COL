...
public class ItemDao {
	...
	public static List<MongoItem> getItemFirst(int catId, int partyId){
		return getItems(catId, partyId, MongoItem.MODID, ASC, 1);
	}
	
	public static List<MongoItem> getItemLast(int catId, int partyId){
		return getItems(catId, partyId, MongoItem.MODID, DESC, 1);
	}
	...
	public static List<MongoItem> getItems(int catId, int partyId, String sortBy){
		return getItems(catId, partyId, sortBy, DESC, 1);
	}
	
	public static List<MongoItem> getItems(int catId, int partyId,String sortBy, int ascOrDescedning, int limit){
		...
		MongoCursor<DBObject> cursor = getItemCollection().find(query).sort(new BasicDBObject(sortBy,ascOrDescedning)).limit(limit).iterator();
